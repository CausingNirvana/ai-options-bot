...
def fetch_atm_iv_and_append(ticker: str):
    ...
    4) Identify the strike closest to current price (ATM).
    5) Read impliedVol (decimal, e.g. 0.2012 → 20.12%).
    ...
    # 5) Read impliedVol (which is decimal, e.g. 0.2012)
    iv_today = float(atm_row["impliedVol"])  # e.g. 0.2012 → 20.12%
...
if __name__ == "__main__":
    # Example usage: python scripts/update_iv_history.py SPY
    import sys
    if len(sys.argv) != 2:
